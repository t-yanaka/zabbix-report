...
function joinQuery(num){
    var d = document.getElementById('joinButtons-id' + num).value;
    var s = d.split("," ,2);
...
function columnsButtons(tables, data1, data2, para1, para2){
     ...
     buf = buf+ "<table border=1 style='color:blue; position:absolute; top:160px; left:250px; font-size: smale;'>";
     ...
     buf=buf+' <div name="bt" class="SecondButtons"> <h3>' + 'join' + '<h3>';
     buf=buf+'<select name="select1">'
     for (var i = 0; i < data2.length; i++) {
         //buf=buf + '<form name="sbt' + i + '"> <p>';
         var keys = Object.keys(data2[i]);
         for (var j = 0; j < keys.length; j++) {
             var d2 = data2[i] [keys[j]];
             //buf = buf + '<form name="bt' + i + '" style="margin: 0px;"> <p>' + '<button type="button" id="joinButtons-id' + i + '" name="manyButtons" value="' + para1 + ',' + d2 + '" onclick="joinQuery(' + i + ')">' + d2 + '</button> </p> </form>';
             buf = buf + '<option value="' + d2  + '">' + d2 + '</option>'
         }
     }
     buf=buf + '</select><br>';
     buf=buf + '<select name="select2">';
     
      var keys = Object.keys(data1[0]); 
     for (var j = 0; j < keys.length; j++) {
         buf=buf + '<option value="' + [keys[j]] + '">' + [keys[j]]  + '</option>'
     }
     buf=buf + '</select><br>';
     buf=buf + '<input type="button" id="manyButtons-"  name="firstButtun" value="送信！" onclick="firstZabbixAjax()">'
     buf=buf + '</div>';
     buf=buf + '<style> div.SecondButtons { color:red; float:left; position:absolute; top:0px; left:250px; } </style>'
     //alert(c)
     $("#id-buttons").html(buf);
}
